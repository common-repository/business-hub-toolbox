...
function bht_insta_feeds( $access_token, $image_num, $disable_cache ) {	

	if( '1' == $disable_cache ){
		$cached = false;
	} else{

		$cached = get_transient( 'bht_instagram_feeds' );
	...
	if ( 200 == $feeds_obj['meta']['code'] ) {
		...
		if ( ! empty( $feeds_obj['data'] ) ) {
			...
			set_transient( 'bht_instagram_feeds', $ending_array, 1 * HOUR_IN_SECONDS );
