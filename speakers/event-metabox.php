 ...
 */
function event_calendar_meta_box_field( $field, $meta = null, $repeatable = null ) {
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				...
				foreach ( $repeatable_fields as $repeatable_field ) {
					...
					echo event_calendar_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
					echo '</p>';
				} // end each field
 ...
 */
function event_calendar_meta_box_find_field_type( $needle, $haystack ) {
	foreach ( $haystack as $h )
		if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
			return event_calendar_meta_box_find_field_type( $needle, $h['repeatable_fields'] );
 ...
 * @return	bool				whether or not the type is in the provided array
 */
function event_calendar_meta_box_find_repeatable( $needle = 'repeatable', $haystack ) {
	foreach ( $haystack as $h )
 ...
 */
function event_calendar_meta_box_santitize_boolean( $string ) {
 ...
 */
function event_calendar_meta_box_sanitize( $string, $function = 'sanitize_text_field' ) {
	switch ( $function ) {
		case 'intval':
 ...
 */
function event_calendar_meta_box_array_map_r( $func, $meta, $sanitizer ) {
	...
	foreach( $meta as $key => $array ) {
		...
		foreach( $array as $arrayKey => $arrayValue )
			if ( is_array( $arrayValue ) )
				$array[$arrayKey] = event_calendar_meta_box_array_map_r( $func, $arrayValue, $newSanitizer[$arrayKey] );
 ...
 */
class Event_calendar_custom_Add_Meta_Box {
    ...
    public function __construct( $id, $title, $fields, $page ) {
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'event_calendar_admin_enqueue_scripts' ) );
		add_action( 'admin_head',  array( $this, 'event_calendar_admin_head' ) );
		add_action( 'add_meta_boxes', array( $this, 'event_calendar_add_box' ) );
		add_action( 'save_post',  array( $this, 'event_calendar_save_box' ));
    }
	
	/**
	 * enqueue necessary scripts and styles
	 */
	function event_calendar_admin_enqueue_scripts() {
		global $pagenow;
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			// js
			$deps = array( 'jquery' );
			if (event_calendar_meta_box_find_field_type( 'date', $this->fields ) )
				$deps[] = 'jquery-ui-datepicker';
			if (event_calendar_meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jquery-ui-slider';
			if (event_calendar_meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			if ( in_array( true, array(
				event_calendar_meta_box_find_field_type( 'chosen', $this->fields ),
				event_calendar_meta_box_find_field_type( 'post_chosen', $this->fields )
			) ) ) {
			...
			if ( in_array( true, array( 
				event_calendar_meta_box_find_field_type( 'date', $this->fields ), 
				event_calendar_meta_box_find_field_type( 'slider', $this->fields ),
				event_calendar_meta_box_find_field_type( 'color', $this->fields ),
				event_calendar_meta_box_find_field_type( 'chosen', $this->fields ),
				event_calendar_meta_box_find_field_type( 'post_chosen', $this->fields ),
				event_calendar_meta_box_find_repeatable( 'repeatable', $this->fields ),
				event_calendar_meta_box_find_field_type( 'image', $this->fields ),
				event_calendar_meta_box_find_field_type( 'file', $this->fields )
			) ) )
			...
			if (event_calendar_meta_box_find_field_type( 'date', $this->fields ) ||event_calendar_meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if (event_calendar_meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
	 ...
	 */
	function event_calendar_admin_head() {
		if ( in_array( get_post_type(), $this->page ) && (event_calendar_meta_box_find_field_type( 'date', $this->fields ) ||event_calendar_meta_box_find_field_type( 'slider', $this->fields ) ) ) {
	 ...
	 */
	function event_calendar_add_box() {
		foreach ( $this->page as $page ) {
			add_meta_box( $this->id, $this->title, array( $this, 'event_calendar_meta_box_callback' ), $page, 'advanced', 'low' );
		}
	}
	 ...
	 */
	function event_calendar_meta_box_callback() {
		...
		foreach ( $this->fields as $field) {
			if ( $field['type'] == 'section' ) {
				...
				echo '<tr>
						...
						echo event_calendar_meta_box_field( $field, $meta );
	 ...
	 */
	function event_calendar_save_box( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
				...
				if ( isset( $new ) && '' == $new && $old ) {
					...
					if ( is_array( $new ) )
						$new = event_calendar_meta_box_array_map_r( 'event_calendar_meta_box_sanitize', $new, $sanitizer );
					else
						$new = event_calendar_meta_box_sanitize( $new, $sanitizer );
